...
CurrentPageNow 	= "attribute_jifensystem.asp"	'当前页.
TitleName 		= "积分名目管理(请勿自行操作！需要修改前请与可乐秀CokeShow.com.cn技术支持中心提前咨询)"	'此模块管理页的名字.
UnitName 		= "积分名目"					'此模块涉及记录的元素名.
'自定义设置.
'本地设置.
Dim CurrentTableName
CurrentTableName 	= "[CXBG_attribute_jifensystem]"	'此模块涉及的[表]名.
'非本地设置.
				...
				<form action="<% = CurrentPageNow %>" method="get" name="deleteAction" id="deleteAction">
					...
					<table width="auto" id="listGo" cellpadding="0" cellspacing="0">
					  ...
					  <tr>
						...
						<td>
                        	<% If rsClass("OperationRule")="+" Then %>
                            <img src="/images/ico/small/coins_add.png" alt="赢取积分" /> 赢取积分
                            <% ElseIf rsClass("OperationRule")="-" Then %>
                            <img src="/images/ico/small/coins_delete.png" alt="扣除积分" /> 扣除积分
                            <% Else %>
                            出错！！！
                            <% End If %>
                        </td>
