...
CurrentPageNow 	= "Questionnaire_ResultView.asp"	'当前页.
TitleName 		= "问卷调查投票结果管理"	'此模块管理页的名字.
UnitName 		= "问卷调查"					'此模块涉及记录的元素名.
'自定义设置.
'本地设置.
Dim CurrentTableName
CurrentTableName 	= "[CXBG_Questionnaire]"	'此模块涉及的[表]名.
'非本地设置.
...
Sub Main()
	...
	sqlClass="SELECT *,coalesce((select distinct count(id) over(partition by byselected_QuestionnaireID) as countBySelectedTotal from [CXBG_Questionnaire_Result] where deleted=0 and byselected_QuestionnaireID="& CurrentTableName &".classid),0) as countBySelectedTotal_Now  ,coalesce((select distinct count(id) over(partition by byask_QuestionnaireID) as countByAskTotal from [CXBG_Questionnaire_Result] where deleted=0 and byask_QuestionnaireID="& CurrentTableName &".classid),0) as countcountByAskTotal_Now From "& CurrentTableName &" ORDER BY RootID,OrderID"
					...
					<table width="auto" id="listGo" cellpadding="0" cellspacing="0">
					  ...
					  <tr>
						...
						<td style="text-align:center;">
							<% If rsClass("Depth")=0 Then %>
								统计为：<font style="color:red;"><% =rsClass("countcountByAskTotal_Now") %></font> 票
                            <% Else %>
                            	<% =rsClass("countBySelectedTotal_Now") %> 票
                            <% End If %>
						</td>
                        
                        <td style="text-align:center;">
						<% If rsClass("Depth")=0 Then %>
						<b style="color: red;"><% =rsClass("jifen") %></b> 积分&nbsp;
                        <% End If %>
                        &nbsp;
                        </td>
						...
						<!--<td style="text-align:right;">
							<% If rsClass("Depth")=0 Then %>
                            <a href="?Action=Add&ParentID=<%=rsClass("id")%>">+向此新增答案</a>
							&nbsp;|&nbsp;
                            <% End If %>
					...
					<table width="auto" id="listGo" cellpadding="0" cellspacing="0">
					  ...
					  <!--<tr>
						...
						<td style="text-align: left;">
							...
							<img style="border:5px #999999 solid;" src="/images/NoPic.png" width="50" height="50" onerror='this.src="/images/NoPic.png"'
							 id="src_photo"
							/></a>
					...
					<table width="auto" id="listGo" cellpadding="0" cellspacing="0">
                      ...
                      <tr>
						...
						<td style="text-align: left;">
                            ...
                            <span style="color:#999999;"> 例如：3+，默认为 0+。设置参与调查将会得到多少积分数。</span>
                      ...
                      <tr>
						...
						<td style="text-align: left;">
							...
							<input type="checkbox" name="isOnpublic" id="isOnpublic" value="1" <% If rsClass("isOnpublic")=1 Then Response.Write "checked=""checked""" %> /><label for="isOnpublic" style="color:#999999;"> 打勾表示<% =UnitName %>将正常显示在网站会员Club栏目上，否则不会在网站显示.</label>
					  ...
					  <!--<tr>
						...
						<td style="text-align: left;">
							...
							<img style="border:5px #999999 solid;" src="<% If rsClass("photo")<>"" Then Response.Write rsClass("photo") Else Response.Write "/images/NoPic.png" %>" width="50" height="50" onerror='this.src="/images/NoPic.png"'
							 id="src_photo"
							/></a>
...
sub SaveModify()
	...
	Else
		If isNumeric(jifen) Then
			rsClass("jifen")=CokeShow.CokeClng(jifen)
		Else
			rsClass("jifen")=0
		End If
	End If
	...
	If isOnpublic="" Or isNull(isOnpublic) Or isEmpty(isOnpublic) Then
		rsClass("isOnpublic")=0
	Else
		If isNumeric(isOnpublic) Then
			rsClass("isOnpublic")=CokeShow.CokeClng(isOnpublic)
		Else
			rsClass("isOnpublic")=0
		End If
	End If
