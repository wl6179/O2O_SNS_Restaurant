...
CurrentPageNow 	= "Account_RemarkOns.asp"			'当前页.
TitleName 		= "会员点评管理"				'此模块管理页的名字.
UnitName 		= "会员点评"					'此模块涉及记录的元素名.
'自定义设置.
'本地设置.
			...
			%>
				<form action="<% = CurrentPageNow %>" method="get" name="deleteAction" id="deleteAction">
					...
					<table width="auto" id="listGo" cellpadding="0" cellspacing="0">
					  ...
					  %>
					  <tr>
						<td colspan="12" style="color:red;">对不起，没有记录...</td>
					  </tr>
					  <%
					  End If
					  %>
					  ...
					  <tr>
						...
						<td style="text-align:center;">
							<a class="abg08" href="/ChineseDish/ChineseDishInformation.Welcome?CokeMark=<% =CokeShow.AddCode_Num( rs222("id") ) %>" target="_blank"  title="<% =rs222("ProductName") %>"><img src="<% If rs222("Photo")<>"" Then Response.Write rs222("Photo") Else Response.Write "/images/NoPic.png" %>" width="40" height="40" /></a>
							&nbsp;
						</td>
						
						<td><%=rs222("ProductName")%>&nbsp;</td>
						
						
						
                        <td>
						<strong style="font-size:14px; font-family: Georgia, 'Times New Roman', Times, serif; color:#F39;"><% =RS("ChineseDish_Taste") %></strong> 分
						&nbsp;
						</td>
                        
                        
                        <td><% =RS("ChineseDish_DiningArea") %>&nbsp;</td>
                        <td><% =RS("ChineseDish_Service") %>&nbsp;</td>
						...
						<td><% =RS("adddate") %>&nbsp;</td>
                        <td><% =RS("Account_LoginID") %>( <% =RS("Account_LoginCNNAME") %> )&nbsp;</td>
                        
						<td>
						
						
						<a href="?Action=Delete&id=<%=RS("id")%>" onClick="return confirm('确定要删除此<% =UnitName %>吗？');">删除屏蔽</a>
						</td>
					  </tr>
                      <tr>
                      	<td colspan="2">&nbsp;</td>
						<td colspan="10" style="color:;">
							<img src="/images/ico/comments2.png" />点评： <% =RS("logtext") %>
                        </td>
					  </tr>
