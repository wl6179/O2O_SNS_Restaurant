...
CurrentPageNow 	= "VIPcardList.asp"	'当前页.
TitleName 		= "VIP卡卡号管理"	'此模块管理页的名字.
UnitName 		= "VIP卡卡号"					'此模块涉及记录的元素名.
'自定义设置.
'本地设置.
Dim CurrentTableName
CurrentTableName 	= "[CXBG_VIPcard]"	'此模块涉及的[表]名.
'非本地设置.
			...
			%>
				<form action="<% = CurrentPageNow %>" method="get" name="deleteAction" id="deleteAction">
					...
					<table width="auto" id="listGo" cellpadding="0" cellspacing="0">
					  ...
					  %>
					  <tr>
						<td colspan="5" style="color:red;">对不起，没有记录...</td>
					  </tr>
					  <%
					  End If
					  %>
					  ...
					  <tr>
                        ...
                        <td style="text-align:center;">
						<% If CokeShow.otherField("[CXBG_account]",rsClass("classname"),"BindingVIPCardNumber","isBindingVIPCardNumber",False,0)=1 Then Response.Write "<img src=/images/yes.gif />已绑定" Else Response.Write "<img src=/images/no.gif />尚无人绑定" %>
                        &nbsp;
                        </td>
                        <td style="text-align:center;">
						<% =rsClass("modifydate") %>
					...
					<table width="auto" id="listGo" cellpadding="0" cellspacing="0">
					  ...
					  <tr>
						...
						<td style="text-align: left;">
							...
							/>-->
                            <% =rsClass("classname") %>
                            <input type="hidden" name="classname" value="<% =rsClass("classname") %>" />
						</td>
					  </tr>
                      ...
                      <tr>
						...
						<td style="text-align: left;">
							...
							<input type="checkbox" name="isOnpublic" id="isOnpublic" value="1" <% If rsClass("isOnpublic")=1 Then Response.Write "checked=""checked""" %> /><label for="isOnpublic" style="color:#999999;"> 打勾表示<% =UnitName %>将正常允许会员进行相应绑定卡号，否则此号码将无法绑定.</label>
