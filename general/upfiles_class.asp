...
CurrentPageNow 	= "upfiles_class.asp"	'当前页.
TitleName 		= "上传图片分类管理模块"	'此模块管理页的名字.
UnitName 		= "上传图片分类"					'此模块涉及记录的元素名.
'自定义设置.
'本地设置.
Dim CurrentTableName
CurrentTableName 	= "[CXBG_upfiles_class]"	'此模块涉及的[表]名.
'非本地设置.
			...
			%>
				<form action="<% = CurrentPageNow %>" method="get" name="deleteAction" id="deleteAction">
					...
					<table width="auto" id="listGo" cellpadding="0" cellspacing="0">
					  ...
					  %>
					  <tr>
						<td colspan="2" style="color:red;">对不起，没有记录...</td>
					  </tr>
					  <%
					  End If
					  %>
					  ...
					  <tr>
						...
						<td>
							<a href="?Action=Add&ParentID=<%=rsClass("id")%>">+向此新增子类</a>
							&nbsp;|&nbsp;
							<a href="?Action=Modify&id=<%=rsClass("id")%>">修改</a>
							&nbsp;|&nbsp;
							<a href="?Action=Move&id=<%=rsClass("id")%>">移动类</a>
							&nbsp;|&nbsp;
					...
					<table width="auto" id="listGo" cellpadding="0" cellspacing="0">
					  ...
					  <tr>
						...
						<td style="text-align: left;">
							...
							dojoType="dijit.form.ValidationTextBox"
							required="true"
							promptMessage="分类名称为必填项."
							invalidMessage="分类长度必须在1-50字之内，例如：商务人士类"
							trim="true"
							lowercase="false"
					...
					<table width="auto" id="listGo" cellpadding="0" cellspacing="0">
					  ...
					  <tr>
						<td style="text-align: right; width: 180px;">
							所属分类：
							<br />
							<a href="?Action=Move&id=<% =id %>">&#187;&#187;移动分类</a>
						</td>
					...
					<table width="auto" id="listGo" cellpadding="0" cellspacing="0">
					  ...
					  <tr>
						<td valign="top">
							...
							<br /><br />
							如果选择复位所有分类，则所有分类都将作为一级分类，这时您需要重新对各个分类进行归属的基本设置。
							<br />
							不要轻易使用该功能，仅在做出了错误的设置而无法复原分类之间的关系和排序的时候使用。
							<br /><br />
						</td>
...
sub SaveReset()
	...
	response.Write "复位成功！请返回<a href='"& CurrentPageNow &"'>分类管理首页</a>做分类的归属设置。"
end sub
