...
CurrentPageNow 	= "supervisor.asp"			'当前页.
TitleName 		= "管理员帐号管理"				'此模块管理页的名字.
UnitName 		= "管理员帐号"					'此模块涉及记录的元素名.
'自定义设置.
'本地设置.
Dim CurrentTableName
CurrentTableName 	= "[CXBG_supervisor]"		'此模块涉及的[表]名.
%>
			...
			Else
				Select Case TypeSearch
					...
					Case "cnname"
						...
						strGuide=strGuide & "中文名中含有“ <font color=red>" & Keyword & "</font> ”的"& UnitName
					Case "username"
				...
				>
					...
					<table width="auto" id="listGo" cellpadding="0" cellspacing="0">
					  ...
					  <tr>
						...
						trim="true"
						onChange="supervisorPasswordsOnChange"
						 value=""
						/>
						</td>
					  </tr>
					  ...
					  <tr>
						<td style="text-align: right;">管理人等级</td>
						<td style="text-align: left;">
						...
						<select name="admin_level" id="admin_level"
						  ...
						  >
							<%
							Call CokeShow.Option_ID("[CXBG_supervisor_class]","",8,0,"classid","classname",True)
							%>
						  </select>
						</td>
			...
			%>
				...
				>
					...
					<table width="auto" id="listGo" cellpadding="0" cellspacing="0">
					  ...
					  <tr>
						<td style="text-align: right;">管理人帐号</td>
						<td style="text-align: left;">
						<% =RS("username") %>
						</td>
						...
						required="false"
						propercase="true"
						promptMessage="请输入您的中文名或者公司的中文名称."
						invalidMessage="中文名称出错..."
						trim="true"
						 value="<% =RS("cnname") %>"
						 style="width: 250px;"
						/>
					  ...
					  <tr>
						...
						<td style="text-align: left;">
						  ...
						  >
							<%
							Call CokeShow.Option_ID("[CXBG_supervisor_class]","",8,RS("admin_level"),"classid","classname",True)
							%>
						  </select>
						</td>
						
						<td style="text-align: right;"><!--是否宽屏操作--></td>
						<td style="text-align: left;">
							<!--<label for="isHaveWork_supervisor1">是</label>
							<input type="radio" name="isHaveWork_supervisor" id="isHaveWork_supervisor1" value="1" <% If RS("isHaveWork_supervisor")=1 Then Response.Write "checked=""checked""" %> />
							&nbsp;&nbsp;&nbsp;
							<label for="isHaveWork_supervisor0">否</label>
							<input type="radio" name="isHaveWork_supervisor" id="isHaveWork_supervisor0" value="0" <% If RS("isHaveWork_supervisor")=0 Then Response.Write "checked=""checked""" %> />-->
						</td>
					  </tr>
...
Sub SaveAdd()
	Dim username,password,repassword,cnname,admin_level
	...
	admin_level	=CokeShow.filtRequest(Request("admin_level"))
	...
	Else
		If CokeShow.strLength(password)>20 Or CokeShow.strLength(password)<6 Then
			FoundErr=True
			ErrMsg=ErrMsg &"<br><li>密码长度不能大于20个字符，也不能小于6个字符（至少六位）！如果你不想修改密码,请保持为空。</li>"
		Else
			password=password
		End If
	End If
	...
	If admin_level="" Then
		FoundErr=True
		ErrMsg=ErrMsg &"<br><li>管理人等级不能为空！</li>"
	Else
		If isNumeric(admin_level) Then
			admin_level=CokeShow.CokeClng(admin_level)
		Else
			FoundErr=True
			ErrMsg=ErrMsg &"<br><li>管理人等级请用数字填写！</li>"
		End If
	End If
...
'记入日志.
Call CokeShow.AddLog("添加操作：成功添加了"& UnitName &"-"& username &"["& cnname &"]", sql)
...
Sub SaveModify()
	Dim password,repassword,cnname,admin_level
	Dim isHaveWork_supervisor
	...
	isHaveWork_supervisor	=CokeShow.filtRequest(Request("isHaveWork_supervisor"))
...
'	End If
	...
	End If
		...
		'RS("isHaveWork_supervisor")	=isHaveWork_supervisor
...
Sub DeleteOne(strID)
	...
	If Not IsObject(CONN) Then link_database
	Set RS=CONN.Execute("SELECT * FROM "& CurrentTableName &" WHERE deleted=0 AND username<>'coke' AND id="& strID)
	...
	Else
		'找不着记录，则
		FoundErr=True
		ErrMsg=ErrMsg & "<br><li>记录id为"& strID &"的"& UnitName &"删除操作没成功，此记录有可能早已丢失，或者您正在删除内置至高帐号coke！</li>"
		Exit Sub
