...
CurrentPageNow 	= "Account_GiftCertificateds.asp"			'当前页.
TitleName 		= "已兑换的礼品券管理"				'此模块管理页的名字.
UnitName 		= "已兑换的礼品券"					'此模块涉及记录的元素名.
'自定义设置.
'本地设置.
Dim CurrentTableName
CurrentTableName 	= "[CXBG_account_GiftCertificated]"		'此模块涉及的[表]名.
%>
...
Sub Main()
	...
	Select Case ExecuteSearch
		Case 0
			sql="SELECT TOP 500 * FROM [View_CXBG_account_GiftCertificated_GiftCertificatedInformation] WHERE deleted=0 ORDER BY id DESC"
				...
				<form action="<% = CurrentPageNow %>" method="get" name="deleteAction" id="deleteAction">
					...
					<table width="auto" id="listGo" cellpadding="0" cellspacing="0">
					  ...
					  <tr>
                        ...
                        <td><span style="<% If DateDiff("d", RS("StopDateValid"), Now())>0 Then %>text-decoration: line-through;<% End If %> color:#FF5383; font-family: Georgia, 'Times New Roman', Times, serif; font-size:18px; "><%=RS("StartDateValid")%></span>&nbsp;</td>
                        <td><span style="<% If DateDiff("d", RS("StopDateValid"), Now())>0 Then %>text-decoration: line-through;<% End If %>color:#FF5383; font-family: Georgia, 'Times New Roman', Times, serif; font-size:18px; font-weight:bold;"><%=RS("StopDateValid")%></span>&nbsp;</td>
                        
                        
                        
                        <td><%=RS("Account_LoginID")%>&nbsp;</td>
                        
                        <td style="text-align:center;"><b style="color: red;"><% =RS("jifen") %></b> 分&nbsp;</td>
